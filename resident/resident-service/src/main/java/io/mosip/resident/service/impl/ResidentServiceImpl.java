...
//			}
			...
			Map<String, BiometricRecord> bios = null;
			try {
				 bios = addBiometricDocuments(dto.getIndividualBiometrics());
			}catch (Exception e) {
			...
			regProcReqUpdateDto.setBios(bios);
	...
	private Map<String, BiometricRecord> addBiometricDocuments(String individualBiometrics) throws Exception {
		Map<String, BiometricRecord> bioValues = new HashMap<String, BiometricRecord>();
		BiometricRecord biometricRecord = new BiometricRecord();
		byte[] data = CryptoUtil.decodeURLSafeBase64(individualBiometrics);
		try {
			cbeffUtil.validateXML(data);
			List<BIR> birs = cbeffUtil.getBIRDataFromXML(data);
			for (BIR bir : birs) {
				biometricRecord.setBirInfo(bir.getBirInfo());
				biometricRecord.setCbeffversion(bir.getCbeffversion());
				biometricRecord.setSegments(bir.getBirs());
				biometricRecord.setVersion(bir.getVersion());
				biometricRecord.setOthers(bir.getOthers());
				bioValues.put(bir.getBdbInfo().getType().toString(), biometricRecord);
			}
